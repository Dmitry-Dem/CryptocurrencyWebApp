...
namespace CWA.Infrastructure.API
{
    public class FallbackCryptoAPI
    {
        ...
        public async Task<string> GetTickersJsonByCurrencyIdAsync(string id)
        {
        ...
        public async Task<string> GetCurrencyHistorycalMarketDataJsonAsync(string id, string days)
        {
            string fileName = "get_currency_prices_by_id-bitcoin_target-usd_days-5_responce.json";

            return await ReadFileAsync(Path.Combine(_basePath, fileName)); ;
        }
        public async Task<string> GetCurrencyPriceJsonByIdAsync(string id, string targetCurrencyId)
        {
            string fileName = "bitcoin_to_usd_price_responce.json";

            return await ReadFileAsync(Path.Combine(_basePath, fileName)); ;
        }
        public async Task<string> GetCurrencyDetailsJsonByIdAsync(string id, string targetCurrencyId)
        {
